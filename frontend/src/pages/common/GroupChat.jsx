    ...
    <div className="w-72 border-l border-slate-100 flex flex-col shrink-0 bg-white">
      ...
      <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-sm text-slate-800">Thành viên</span>
          {data?.members && (
        ...
        <div className="mx-3 my-3 p-3 rounded-xl border border-primary-100 bg-primary-50/40">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-primary-700 flex items-center gap-1">
              <BadgeCheck className="w-3.5 h-3.5" /> Biệt danh của bạn
            </span>
            {!editing && (
              <button
          ...
          {editing ? (
            ...
            <p className="text-sm font-semibold text-slate-700">
              {myMember?.nickname ? (
                ...
                <span className="text-slate-400 italic text-xs">Chưa đặt biệt danh</span>
              )}
            </p>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1.5">
        {loading ? (
          ...
          <p className="text-xs text-center text-slate-400 pt-6">Không có thành viên.</p>
        ) : (
  ...
  if (loadingChannels) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh]">
        ...
        <p className="text-sm text-slate-400 mt-2 font-medium">Đang tải chat...</p>
      </div>
    );
  }
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {filteredChannels.length === 0 ? (
            ...
            filteredChannels.map((c) => {
              ...
              const lastMsgText = c.lastMessage?.text || 'Chưa có tin nhắn';
      ...
      <div className="flex-1 flex flex-col bg-slate-50/20 min-w-0">
        {selectedChannel ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-mono mt-0.5">
                    Lớp: {selectedChannel.class?.classCode || '—'} · {selectedChannel.team?.teamCode || 'General'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {loadingMessages ? (
                  <div className="flex flex-col items-center justify-center h-full">
                    ...
                    <p className="text-xs text-slate-400 mt-1 font-medium">Đang tải tin nhắn...</p>
                  </div>
                ) : messages.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-slate-300">
                    <MessageSquare className="w-16 h-16 stroke-1 mb-2 animate-bounce" />
                    <p className="text-sm font-semibold">Chào mừng đến nhóm chat!</p>
                    <p className="text-xs text-slate-400 mt-0.5">Hãy bắt đầu cuộc trò chuyện với nhóm.</p>
                  </div>
                ) : (
                              ...
                              <div className="mt-2 max-w-xs">
                                {m.attachment.fileType === 'image' ? (
                                  ...
                                  >
                                    ...
                                    <span className="truncate max-w-[150px]">{m.attachment.name || 'Tải file'}</span>
                                  </a>
                                )}
                              </div>
                            )}
            ...
            <div className="p-4 bg-white border-t border-slate-100 shrink-0 space-y-2">
              ...
              <form onSubmit={handleSendMessage} className="flex gap-2">
                ...
                <input
                  ...
                  placeholder={selectedFile ? 'Thêm tin nhắn hoặc nhấn Gửi...' : 'Nhập tin nhắn...'}
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-300">
            ...
            <p className="text-sm font-semibold">Chưa chọn kênh chat</p>
          </div>
        )}
      </div>
